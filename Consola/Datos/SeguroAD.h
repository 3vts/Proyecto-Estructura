...
int eliminarSeguro(string cedula){
	...
	try {
		...
		const char *sqlInsert = "DELETE FROM Seguros WHERE Cedula = ?";
		int result = sqlite3_prepare(db, sqlInsert, strlen(sqlInsert), &compiledStatement, 0);
		if(result) throw invalid_argument(sqlite3_errmsg(db));
...
void crearTablaSeguro(){
	...
	try {
		...
		const char *sqlCreateTable = "CREATE TABLE IF NOT EXISTS Seguros (cedula STRING PRIMARY KEY, tipo_auto STRING,  monto_anual INTEGER)";
...
Seguro obtenerSeguroPorCedula(string cedula) {
	sqlite3 *db;
	sqlite3_stmt *compiledStatement;
	try {
		sqlite3_open(DB, &db);
		...
		if (res == SQLITE_ROW) {
            ...
            Seguro seguro = Seguro(cedula, tipo_auto, monto_anual);
            sqlite3_finalize(compiledStatement);
            sqlite3_close(db);
            return seguro;
        } else {
        	throw invalid_argument("Seguro no existe");
		}
